...
class SeqEval:
    ...
    def extract_entities(self,label_lst):
    ...
    def split_index(self,label_list):
        label_dict = self.label_dict
        label_list = self.label_list
    ...
    def compute_performance_eachq(self,doc_id_lst, input_ids, doc_tokens, input_masks, pred_labels, gold_labels, label_masks,
                                  input_types, entity_types, tokenizer=None, result_dict={},
                                  gold_relation=[],valid_lst=None):
        '''
        ...
        :return:
        '''
        label_map = self.label_list
        label_list = self.label_list

        num_ques = len(input_ids[0])
        ent_accuracy, ent_positive, ent_extracted, ent_true = [0] * num_ques, [0] * num_ques, [0] * num_ques, [
            0] * num_ques
        rel_accuracy, rel_positive, rel_extracted, rel_true = [0] * num_ques, [0] * num_ques, [0] * num_ques, [
            0] * num_ques
        num_rel, num_ent = 0, 0
        logs = []
        for doc_id, every_input_ids, every_doc_tokens, every_input_masks, every_pred_label, every_golden_label, every_label_masks, every_input_type, every_entity_type in \
                zip(doc_id_lst, input_ids, doc_tokens, input_masks, pred_labels, gold_labels, label_masks, input_types,
                    entity_types):
            ...
            truth_entities = self.extract_entities(gold_label_ids)
            ...
            for pred_label_i in every_pred_label:  # pred_label_i: max_seq
                ...
                pred_entities = self.extract_entities(pred_label_ids)  # {"start_end":[ids]}
        ...
        for ques_idx in range(num_ques):
            ent_acc, ent_precision, ent_recall, ent_f1 = compute_f1(ent_accuracy[ques_idx], ent_positive[ques_idx],
                                                                    ent_extracted[ques_idx], ent_true[ques_idx],
                                                                    num_ent)
            rel_acc, rel_ent_precision, rel_recall, rel_f1 = compute_f1(rel_accuracy[ques_idx], rel_positive[ques_idx],
                                                                        rel_extracted[ques_idx], rel_true[ques_idx],
                                                                        num_rel)

            ent_results.append({"accuracy": ent_acc, "precision": ent_precision, "recall": ent_recall, "f1": ent_f1})
            rel_results.append(
                {"accuracy": rel_acc, "precision": rel_ent_precision, "recall": rel_recall, "f1": rel_f1})

        return {"entity": ent_results,
                "relation": rel_results}, logs, result_dict

    def compute_performance(self,doc_id_lst, input_ids, doc_tokens, input_masks, pred_labels, gold_labels, label_masks,
                            input_types, entity_types,  tokenizer=None,
                            ent_weight=[1, 1, 1], rel_weight=[1, 1, 1], result_dict={}, gold_relation=[],valid_lst=None):
        '''
        :param input_ids: num_all_example, 3, max_seq
        :param doc_tokens: num_all_example, max_seq
        :param input_masks: num_all_example, 3, max_seq
        ...
        for doc_id, every_input_ids, every_doc_tokens, every_input_masks, every_pred_label, every_golden_label, every_label_masks, every_input_type, every_entity_type, every_gold_relation in \
            ...
            final_pred_entities =  self.extract_entities(final_pred_label)
            ...
            if tokenizer is not None:
                ...
                if (every_input_type == 'entity'):
                    ...
                    head_entity = self.get_head_entity(every_entity_type, log["questions"][0])
        ...
        ent_accuracy, ent_precision, ent_recall, ent_f1 = compute_f1(ent_accuracy, ent_positive, ent_extracted,
                                                                     ent_true, num_ent)
        rel_accuracy, rel_ent_precision, rel_recall, rel_f1 = compute_f1(rel_accuracy, rel_positive, rel_extracted,
                                                                         rel_true, num_rel)
        print("ent:ent_acc:{},ent_positive:{}, ent_extracted：{}, ent_true：{}, num_ent：{}".format(ent_accuracy,
                                                                                                 ent_positive,
                                                                                                 ent_extracted,
                                                                                                 ent_true, num_ent))
        print("rel:rel_acc:{},rel_positive:{}, rel_extracted：{}, rel_true：{}, num_rel：{}".format(rel_accuracy,
                                                                                                 rel_positive,
                                                                                                 rel_extracted,
                                                                                                 rel_true, num_rel))

        return {"entity": {"accuracy": ent_accuracy, "precision": ent_precision, "recall": ent_recall, "f1": ent_f1},
                "relation": {"accuracy": rel_accuracy, "precision": rel_ent_precision, "recall": rel_recall,
                             "f1": rel_f1}}, logs, result_dict

    def compute_result_dict(self,result_dict):
        # print(result_dict)
    ...
    def generate_relation_examples(self,doc_id_lst, input_ids, doc_tokens, input_masks, pred_labels, gold_labels,
                                   label_masks, entity_types, golden_relations, config, tokenizer,
                                   ent_weight=[1, 1, 1], rel_logits_lst=[], logger=None, unused=False):
        '''
        :param input_ids: num_all_example, 3, max_seq
        :param doc_tokens: num_all_example, max_seq
        :param input_masks: num_all_example, 3, max_seq
        ...
        for every_doc_id, every_input_ids, every_doc_tokens, every_input_masks, every_pred_label, every_golden_label, every_label_masks, entity_type, golden_relation, rel_logits in \
                tqdm(zip(doc_id_lst, input_ids, doc_tokens, input_masks, pred_labels, gold_labels, label_masks,
                         entity_types, golden_relations, rel_logits_lst)):
            # batch_doc: 3, max_seq
            # batch_pred_label: 3, max_seq
            # extract gold_label
            ...
            for key, indicator in final_pred_entities.items():
                ...
                if (unused):
                    every_doc_tokens1 = every_doc_tokens[:start_idx] + ['[unused0'] + every_doc_tokens[
                                                                                      start_idx:end_idx + 1] + [
                                            '[unused1'] + every_doc_tokens[end_idx + 1:]
                else:
                ...
                if (use_filter_flag > 0):  # use filter
                    ...
                    for i, rel_flag in enumerate(rel_logits):  # live_in, work_for, located_in ...
                        ...
                        if (rel_flag):
                            ...
                            if (config.dataname == 'conll04'):
                                ...
                                # print(questions)
                                # 获得真实答案
                                label, ans_num =self.extract_relation_answer(golden_relation, relation,
                                                                         orig_head_entity_text,
                                                                         len(every_doc_tokens))
                                if (unused):
                                    label1 = label[:start_idx] + ['O'] + label[start_idx:end_idx + 1] + ['O'] + label[
                                                                                                                end_idx + 1:]
                                else:
                                    label1 = label
                                example = MRCExample(
                                ...
                                if (config.use_filter_flag == 1):
                                    for ent_type in ent_label_list:
                                        ...
                                        # 从对应的关系生成关系特定的问题
                                        for relation_template in question_templates[
                                            str((entity_type, relation, ent_type))]:  # 3 questions
                    ...
                    for relation in entity_relation_map[entity_type]:  # live_in, work_for, located_in ...
                        ...
                        if (config.dataname == 'ace2005'):
                            ...
                            # logger.info(relation)
                        label, ans_num = self.extract_relation_answer(golden_relation, relation, orig_head_entity_text,
                                                                 len(every_doc_tokens))
        ...
        # print(all_count, q)
        logger.info(
            "have_ans:{}/{},total_ans_num:{},seo_num:{}/{},pred_total_ent:{}".format(total_have_ans_num, len(examples),
                                                                                     total_ans_num, total_seo_num,
                                                                                     len(examples), total_ent_num))
        return examples

    def extract_relation_answer(self,golden_relations, relation, head_entity_text, len_doc):
        tail_entities = []
    ...
    def get_head_entity(self,every_entity_type, q):
        q_list = q.split(' ')
        if (type == 'kill'):
...
class MRCTPlinkerEval:
    def __init__(self,label_list):
        self.ent_label_list,self.rel_label_list = label_list
        self.ent_label_dict = {label: i for i, label in enumerate(self.ent_label_list)}
        self.rel_label_dict = {label: i for i, label in enumerate(self.rel_label_list)}


    def extract_entities(self, label_lst):
        rel_label_dict = self.rel_label_dict
    ...
    def compute_performance_eachq(self,doc_id_lst, input_ids, doc_tokens, input_masks, pred_labels, gold_labels, label_masks,
        ...
        :return:
        '''
        rel_label_map = self.rel_label_dict
        ...
        num_rel, num_ent = 0, 0
        logs = []
        for doc_id, every_input_ids, every_doc_tokens, every_input_masks, every_pred_label, every_golden_label, every_label_masks, every_input_type, every_entity_type,every_valid in \
                zip(doc_id_lst, input_ids, doc_tokens, input_masks, pred_labels, gold_labels, label_masks, input_types,
                    entity_types,valid_lst):
            seq_len=sum(valid_lst)
            # print(seq_len)
            mask_index = [tmp_idx for tmp_idx, tmp in enumerate(every_label_masks) if tmp != 0]
            gold_label_ids = [tmp for tmp_idx, tmp in enumerate(every_golden_label) if tmp_idx in mask_index]
            gold_label_ids = gold_label_ids[1:-1]
            ...
            for pred_label_i in every_pred_label:  # pred_label_i: max_seq
                ...
                for key, ids in final_pred_entities.items():
                    try:
                        ...
                        print(ids)
                pred_label = iob2_to_iobes(pred_label)
                gold_label = [rel_label_map[l] for l in gold_label_ids]
                assert len(gold_label) == len(pred_label)
