...
class conll04PreProcess:
        ...
        for t in data2:
            ...
            for ent_type in ent_label_list:
                if (self.ent_matrix_label):
                    if (self.mix_ent_rel and self.rel_tail_head):
    ...
    def get_train_dev(self,input_file,input_file2,path):
        """Read a SQuAD json file into a list of SquadExample."""
        with open(input_file, "r", encoding='utf-8') as reader:
            input_data = json.load(reader)["data"][0]
        with open(input_file2, "r", encoding='utf-8') as reader:
            input_data2 = json.load(reader)["data"][0]
        result={"data":[{"title":"conll04",
                                "paragraphs":input_data["paragraphs"]+input_data2["paragraphs"]}]}
        print(len(input_data["paragraphs"]),len(input_data2["paragraphs"]))
        print(len(result["data"][0]["paragraphs"]))
        with open(path, 'w', encoding='utf-8') as f:
            json.dump(result, f)
    ...
    def process(self):
        ...
        for name in file_name:
            ...
            if(self.model=='mrc4ere'):
                ...
                self.get_mrctp_json(data, self.output_dir + name + '_epo.json', name="epo")
                self.get_mrctp_json(data, self.output_dir + name + '_spo.json', name="seo")
                self.get_mrctp_json(data, self.output_dir + name + '_normal.json', name="normal")
        self.get_train_dev(self.output_dir+'train.json',self.output_dir+'dev.json',self.output_dir+'train_dev.json')
import argparse
import pickle
...
if(args.model=='mrctplink'):
    ...
    args.output_dir=args.model+('_matrix' if args.ent_matrix_label else '')+('_rel_head_tail' if args.rel_tail_head else '')+'/'
