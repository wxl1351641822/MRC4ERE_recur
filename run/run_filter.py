...
def eval_checkpoint(model_object, eval_features, config, device, n_gpu, label_list, eval_sign="dev", tokenizer=None,
    ...
    if eval_sign == "dev":
        loss, input_lst, doc_token_lst, input_mask_lst, pred_lst, gold_lst, label_mask_lst, type_lst, etype_lst, gold_relation = evaluate(
            model_object,
        ...
        _, ent_input_lst, ent_doc_lst, ent_input_mask_lst, ent_pred_lst, ent_gold_lst, ent_label_mask_lst, ent_type_lst, ent_etype_lst, ent_gold_relation = evaluate(
            model_object,
            eval_features,
            config,
            device,
        ...
        relation_examples = generate_relation_examples(ent_input_lst, ent_doc_lst, ent_input_mask_lst, ent_pred_lst,
                                                       ...
                                                       ent_weight)  # batch x 3 x max_seq_len
        ...
        if len(relation_features) > 0:
            _, rel_input_lst, rel_doc_lst, rel_input_mask_lst, rel_pred_lst, rel_gold_lst, rel_label_mask_lst, rel_type_lst, rel_etype_lst, rel_gold_relation = evaluate(
...
def evaluate(model_object, eval_features, config, device, eval_sign="dev"):
    ...
    for batch_i in tqdm(range(num_batches)):
        ...
        with torch.no_grad():
            ...
            logits = model_object(input_ids, token_type_ids=segment_ids, attention_mask=input_mask, valid_ids=valid_ids,
...
def main():
    ...
    train(tokenizer, model, optimizer, train_loader, dev_loader, test_loader, config, device, n_gpu, label_list,
          num_train_steps)
